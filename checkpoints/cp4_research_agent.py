#!/usr/bin/env python3
# CHECKPOINT 4 build - Prompt 4 "the checks".
# This stage adds an evaluation mode: python research_agent.py --eval
# After a normal run, five PASS/FAIL checks are printed with a total score:
...
#
# The checks read the run's state and output - NOT the model's opinion of
# itself. A model asked to mark its own work will nearly always say it did
# well, so these checks look at what actually happened instead.
...
def run_agent(question):
    ...
    # the state so the model can see it on the next step, and carry on.
    state = []
    pages_read = []
    finished = False
    report = None
    ...
    return state, pages_read, report, finished, steps_used
...
def evaluate(question):
    ...
    # search, consult sources, finish in budget, recommend something, and
    # list at least three sources), not whether the answer is factually
    # correct - none of these checks could know that.
    state, pages_read, report, finished, steps_used = run_agent(question)
    distinct_read = set(pages_read)

    # Count the URLs the brief actually lists, as the "three sources" check.
    source_urls = re.findall(r"https?://\S+", report or "")
